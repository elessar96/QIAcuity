...
classify_peaks <- function(turnpoints, intensities, variable, channel_maxima, reference_peaks=NULL){
  ...
  peak_info <- peak_info %>% na.omit() %>% group_by(peak) %>% summarize_at(.vars = colnames(intensities), .funs= c("min"=min, "median"=median, "max"=max), na.rm=TRUE) %>% na.omit() %>% mutate_if(is.numeric, round, digits=1)
  ...
  if(!is.null(reference_peaks)){
    ...
    peak_info <- peak_info %>% mutate(tn.peak.dist = ifelse(dist.tn == dist.tn %>% abs() %>% min(., na.rm=TRUE), 1, 0)) %>% mutate(tp.peak = ifelse(dist.tp == dist.tp %>% abs() %>% min(., na.rm=TRUE), TRUE, FALSE))
    
    tn_candidates <- peak_info %>% filter(height > max(height)/5000) %>% filter(dist.tp > dist.tn) %>% mutate(rank_height = rank(height)) %>% mutate(rank=tn.peak.dist*2 + rank_height) %>% filter(rank == max(rank))
    tp_candidates <- peak_info %>% filter(dist.tp < dist.tn) %>% filter(tp.peak==TRUE) %>% filter(dist.tn > abs(reference_peaks[1,]$d.x - reference_peaks[2,]$d.x)/4) %>% arrange(peak) %>% mutate(rank_dist.tn =rank(-dist.tn)) %>% mutate(rank_dist.tp =rank(dist.tp)) %>% mutate(max_others = rank(max_others)) %>% mutate(rank = max_others + rank_dist.tp) %>% filter(rank == min(rank, na.rm=TRUE))
    ...
    if(nrow(peak_info)>2){
      ...
      if(length(other_channels)==0){
        tn_candidates <- tn_candidates %>% arrange(peak) %>% mutate(position = rank(abs(as.numeric(position)))) %>% mutate(rank = position) %>% filter(rank == min(rank, na.rm=TRUE))
      }else{
        ...
        tn_candidates <- non_ct_peaks %>% arrange(peak)  %>% mutate(max_others = rank(max_others)) %>% mutate(position = rank(abs(as.numeric(position)))) %>% mutate(rank = max_others + position) %>% filter(rank == min(rank, na.rm=TRUE)) %>% filter(position==min(position, na.rm=TRUE))
      }
      tp_candidates <- peak_info %>% filter(height > height %>% max()/100) %>% filter(position > (peak_info %>% filter(peak %in% tn_candidates$peak) %>% filter(height > height %>% max()/30)  %>% pull(position) %>% max())) %>% filter(!peak %in% c(tn_candidates %>% filter(position==min(position, na.rm=TRUE)) %>% pull(peak))) %>% arrange(peak) %>% mutate(pos = peaks$d.x[.$peak]) %>% mutate(max_others = rank(max_others)) %>% mutate(position = rank(-position)) %>% mutate(rank = max_others + position) %>% filter(rank == min(rank, na.rm=TRUE)) 
    }else{
  ...
  tn_peak <- peaks[tn_candidates %>% filter(max_others==min(max_others, na.rm=TRUE)) %>% filter(position==min(position, na.rm=TRUE)) %>% pull(peak) %>% as.character() %>% gsub("peak ", "", .) %>% as.numeric(),]
  
  tp_peak <- peaks[tp_candidates %>% filter(position==min(position, na.rm=TRUE)) %>% filter(max_others==min(max_others, na.rm=TRUE)) %>% pull(peak) %>% as.character() %>% gsub("peak ", "", .) %>% as.numeric(),]
