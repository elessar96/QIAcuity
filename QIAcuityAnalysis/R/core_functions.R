...
QIAcuityAnalysis <- function(input_path, output_path, smooth=FALSE){
  ...
  setwd(input_path)
  writeLines("QIAcuity data analysis started", con="Log.txt")
  
  # for each experiment, do:
  for(k in 1:length(plates)){
    setwd(output_path)
    #print(plates[k])
    readLines(con="Log.txt") %>% append(c(plates[[k]])) %>% writeLines(., con="Log.txt")
    ...
    pc_wells <- maxima %>% mutate(across(.cols=all_of(channels), .fns=function(x){return(x-min(x))})) %>% mutate(across(.cols=all_of(channels), .fns=function(x){return(x/max(x))}))  %>% mutate(minimum = apply(across(all_of(channels)), 1, min)) %>% mutate(median = apply(across(all_of(channels)), 1, median)) %>% filter(median == max(median) | minimum == max(minimum) | median == min(median)) %>% pull(Well) %>% unique()
    ...
    if(class(baseline_corrected) =="try-error"){
      readLines(con="../Log.txt") %>% append(c("Error: baseline correction failed. Message:", geterrmessage())) %>% writeLines(., con="../Log.txt")
    ...
    for(i in 1:length(channels)){
      ...
      peak_data <- tp_data %>% filter(!!sym(channels[[i]])>lower_lim) %>% filter(!!sym(channels[[i]])<upper_lim) %>% summarize_at(channels[-i], median)
    ...
    if(class(temp) =="try-error"){
      readLines(con="../Log.txt") %>% append(c("Error: crosstalk correction failed. Message:", geterrmessage())) %>% writeLines(., con="../Log.txt")
    ...
    if(class(temp) =="try-error"){
      readLines(con="../Log.txt") %>% append(c("Error: competition correction failed. Message:", geterrmessage())) %>% writeLines(., con="../Log.txt")
    ...
    temp <- recalculate_thresholds(data=competition_corrected, thresholds=thresholds, step="competition", coupled_channels = coupled_channels, min_dist=0.4, mid=TRUE)
    ...
    readLines(con="../Log.txt") %>% append(c("Analysis completed.")) %>% writeLines(., con="../Log.txt")
    #}
    ...
    vol_by_well = dataframes1 %>% bind_rows() %>% mutate(`Is invalid` = ifelse(`Is invalid` == "0", "valid", "invalid")) %>% group_by(Well, `Cycled volume`, Channel, `Is invalid`) %>% summarize(N_total = n()) %>% pivot_wider(names_from= `Is invalid`, values_from=N_total) %>% mutate(`Cycled volume` = as.numeric(`Cycled volume`)*(valid/(invalid + valid)))
    ...
    for(j in 1:length(corrections)){
      ...
      for(k in 1:n_channels){
          ...
          for(m in 1:n_channels){
            if(!k>=m){
              if(n_channels>2){
                ...
                scatterplot <- results[[i]][[corrections[[j]]]] %>% dplyr::select(Well | ends_with(results[[i]]$thresholds$channel[c(m, k, p)])) %>% na.omit() %>% ggplot(aes(x =!!sym(results[[i]]$thresholds$channel[[m]]), y =!!sym(results[[i]]$thresholds$channel[[k]]), color=!!sym(results[[i]]$thresholds$channel[[p]]))) + geom_point()
                if(!j==1){
        ...
        if(scatterplots_1d==TRUE){
          plot <- results[[i]][[corrections[[j]]]]  %>% dplyr::select(Well | Partition | ends_with(results[[i]]$thresholds$channel[k])) %>% na.omit() %>% mutate(Partition = Partition %>% as.numeric()) %>% mutate(across(Well, ~ factor(., levels=c(well_order)))) %>% ggplot(aes(x=Partition, y =!!sym(results[[i]]$thresholds$channel[[k]]))) + geom_jitter(size=0.5) + scale_x_discrete(limits=well_order) + facet_wrap(vars(Well), nrow =2) + theme(axis.text.x=element_blank(), axis.ticks.x=element_blank())
        ...
        if(j>1){
          ...
          zscore_RFU_pos <- results[[i]][[corrections[[j]]]] %>% filter(!!sym(temp_res[[i]]$thresholds$channel[[k]])>results[[i]]$thresholds[k, corrections[[j]] %>% gsub("_corrected", "", .)]) %>% mutate(z_score = (!!sym(temp_res[[i]]$thresholds$channel[[k]])-mean_RFU_pos)/sd_RFU_pos) %>% group_by(Well) %>% summarize_at("z_score", mean)
      ...
      if(!j==1){
        ...
        channel_summaries <- merge(channel_summaries, results[[i]]$Volumes %>% select(Well, `Cycled volume`, Channel), by=c("Well", "Channel")) %>% mutate(factor = (!!sym("Cycled volume") %>% as.numeric())/(mm_vol*as.numeric(valid)))
