...
ui <- fluidPage( # Application title
  ...
  mainPanel(
    ...
    fluidRow(strong("Currently selected input path:")),
    ...
    checkboxInput("out_in_equiv", label="Output folder same as input folder?"),
...
server <- function(input, output) {
  root <- getwd() %>% dirname() %>% dirname() %>% dirname() %>% dirname() %>% dirname()
  ...
  output$done <- renderText({
    if(input$analyse==TRUE){
      ...
      result <- QIAcuityAnalysis(input_path=input_path, output_path=output_path, smooth=TRUE) #coupled_channels = coupled,
